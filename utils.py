...
def generate_entrie_images(img_origin, img_grad, img_grad_overlay, img_integrad,
                           img_integrad_overlay):
    blank = np.ones((img_grad.shape[0], 10, 3), dtype=np.uint8) * 255
    blank_hor = np.ones((10, 20 + img_grad.shape[0] * 3, 3), dtype=np.uint8) * 255
    upper = np.concatenate([img_origin[:, :, (0, 1, 2)], blank, img_grad_overlay, blank, img_grad],
                           1)
    down = np.concatenate(
        [img_origin[:, :, (0, 1, 2)], blank, img_integrad_overlay, blank, img_integrad], 1)
